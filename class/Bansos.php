<?php
class Bansos extends Database {
public function tampil(){
// 1. mysqli_query
$sql=$this->mysqli->query("SELECT * FROM bansos") or die
($this->CekError());
...
// 1. mysqli_query
$sql=$this->mysqli->query("SELECT * FROM bansos WHERE
id_bantuan='".$id_bantuan."'") or die ($this->CekError());
$dataBansos=$sql->fetch_object();
// 2. jika datanya ada
if(isset($dataBansos)){
...
function update($id_bantuan,$data){
...
$this->mysqli->query("UPDATE bansos SET ".$script_update."
WHERE id_bantuan='".$id_bantuan."'") or die ($this->CekError());
// 4. tutup koneksi
$this->TutupKoneksi();
}
function hapus($id_bantuan){
// 1. Jalankan perintah delete query
$this->mysqli->query("DELETE FROM bansos WHERE
...
$sql_simpan="INSERT INTO bansos (".$kolom_nya_baru.")
